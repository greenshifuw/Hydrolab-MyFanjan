...
import { Activity, Zap, Sparkles, PlusCircle, MinusCircle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ state, tankCapacity, onUpdate, onBuyNutrient, onDrainNutrient, advice, onRefreshAdvice, optimScore }) => {
    ...
    <div className="flex flex-col gap-4 sm:gap-6 shrink-0">
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        ...
        <div className={`p-3 sm:p-4 rounded-2xl sm:rounded-3xl border-2 shadow-sm transition-all duration-500 ${optimScore > 80 ? 'bg-green-50 border-green-100' : 'bg-orange-50 border-orange-100'}`}>
          <p className="text-[7px] sm:text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Efficacité Bio</p>
      ...
      <div className="space-y-2 sm:space-y-3">
        <h4 className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 px-1">
            <Activity className="w-3 h-3" /> Nutriments (NPK)
        </h4>
        <div className="grid grid-cols-1 gap-2">
          <NutrientDosage label="Azote (N)" value={state.currentN} color="bg-blue-500" onAdd={() => onBuyNutrient('currentN')} onRemove={() => onDrainNutrient('currentN')} />
          <NutrientDosage label="Phosphore (P)" value={state.currentP} color="bg-purple-500" onAdd={() => onBuyNutrient('currentP')} onRemove={() => onDrainNutrient('currentP')} />
          <NutrientDosage label="Potassium (K)" value={state.currentK} color="bg-orange-500" onAdd={() => onBuyNutrient('currentK')} onRemove={() => onDrainNutrient('currentK')} />
        </div>
      </div>

      <div className="space-y-2 sm:space-y-3">
        <h4 className="text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Maintenance du Réservoir</h4>
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
            <ControlSlider label="pH" value={state.currentPH} min={4} max={9} step={0.1} unit="" color="bg-teal-500" onChange={(v) => onUpdate('currentPH', v)} />
            <ControlSlider label="EC Target" value={state.currentEC} min={0} max={4} step={0.1} unit=" mS" color="bg-indigo-500" onChange={(v) => onUpdate('currentEC', v)} />
            <ControlSlider label="Température" value={state.currentTemp} min={10} max={35} unit="°C" color="bg-red-500" onChange={(v) => onUpdate('currentTemp', v)} />
            <ControlSlider label="Eau (30L)" value={state.currentWater} min={0} max={100} unit={`% (${waterLiters.toFixed(1)}L)`} color="bg-blue-400" onChange={(v) => onUpdate('currentWater', v)} />
        </div>
      </div>
      ...
      <div className="bg-slate-900 rounded-[1.5rem] sm:rounded-[2rem] p-4 sm:p-5 text-white shadow-xl border border-slate-700 mt-1 shrink-0">
        <div className="flex justify-between items-center mb-3 sm:mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[8px] sm:text-[9px] font-black text-blue-400 uppercase tracking-widest">IA Hydro-Logiq</h3>
            </div>
        ...
        <p className="text-[10px] sm:text-[11px] font-medium leading-relaxed text-slate-300 italic min-h-[30px]">
          {typedAdvice ? `"${typedAdvice}"` : "Analyse du métabolisme en cours..."}
        </p>
      </div>
    </div>
  );
...
const NutrientDosage: React.FC<{ label: string, value: number, color: string, onAdd: () => void, onRemove: () => void }> = ({ label, value, color, onAdd, onRemove }) => (
  <div className="bg-white p-2 sm:p-3 rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3 group">
    ...
    <div className="flex gap-1 shrink-0">
        <button onClick={onRemove} className="flex flex-col items-center justify-center bg-slate-50 active:bg-red-50 w-9 h-9 sm:w-11 sm:h-11 rounded-xl transition-all border border-slate-100">
        ...
        <button onClick={onAdd} className="flex flex-col items-center justify-center bg-slate-50 active:bg-blue-50 w-9 h-9 sm:w-11 sm:h-11 rounded-xl transition-all border border-slate-100">
...
const ControlSlider: React.FC<{ label: string, value: number, min: number, max: number, step?: number, unit: string, color: string, onChange: (v: number) => void }> = ({ label, value, min, max, step = 1, unit, color, onChange }) => (
  <div className="bg-white p-2 sm:p-3 rounded-xl sm:rounded-2xl border border-slate-100 shadow-sm">
    <div className="flex justify-between items-center mb-1">
    ...
    <div className="relative flex items-center h-4">
        <input 
          ...
          className="w-full h-1 rounded-lg appearance-none cursor-pointer accent-slate-900 bg-slate-100"
        />
    </div>
  </div>
);
